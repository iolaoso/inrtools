console.log("listFilesProvisiones.js funcionando");
...
function mostrarErrorEnTodasTablas(mensaje) {
    ['repTableBodyProv123', 'repTableBodyProv45'].forEach(id => {
...
// Función reportes Provisiones
function fetchProvisiones(carpetaReportes) {
    const url = `${baseurl}/backend/reportes/listFilesProvisiones.php?carpeta=${encodeURIComponent(carpetaReportes)}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            const categorias = {
                perdidasEsperadas: {
                    archivos: data.filter(item => item.name.includes('REPORTE_PROVISIONES_S123')),
                    tablaId: 'repTableBodyProv123'
                },
                histPerdidasEsperadasMT: {
                    archivos: data.filter(item => item.name.includes('REPORTE_PROVISIONES_S45')),
                    tablaId: 'repTableBodyProv45'
                }
            };
...
document.addEventListener("DOMContentLoaded", () => {
    if (typeof carpetaReportes !== 'undefined') {
        fetchProvisiones(carpetaReportes);
    } else {
