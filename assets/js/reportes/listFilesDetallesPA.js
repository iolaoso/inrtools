console.log('Carpeta de reportes:', carpetaReportes);
...
function mostrarErrorEnTodasTablas(mensaje) {
    ['rTBodyPA', 'rTBodyHalRecEst'].forEach(id => {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            const categorias = {
                ...
                HalRecEst: {
                    archivos: data.filter(item => item.name.includes('2.14. DETALLE_HAL_REC_EST')),
                    tablaId: 'rTBodyHalRecEst'
                }
            };
